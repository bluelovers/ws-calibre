...
function _concat(t1: string, t2: string, link: string, prop: string, fields: string[], sep = '|')
{
	const [f1, f2] = fields;
	const c = `${t1}_${prop}`;
...
function concat_simple(t: string, t2: string, f: string, prop: string, distinct = true)
{
	// @ts-ignore
...
function concat(f1: string, f2: string, prop, dir = false, sep = '|')
{
...
export class Statement
{
	...
	sumBooks<T extends EnumBooksFilter.author | EnumBooksFilter.tag | EnumBooksFilter.series>(_from = EnumBooksFilter.author as T)
	{
		this._query
